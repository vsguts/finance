...
$(document).on('click', function(e) {
    ...
    var elm = jelm.closest('[data-form-input]');
    if (elm.length) {
        var name = elm.data('form-input'),
            value = elm.data('value') || '',
            formId = elm.attr('form');
        if (name && formId) {
            var form = $('#'+formId);
            form.find('[data-append]').remove();
            form.append("<input name='" + name + "' value='" + value + "' class='hidden' data-append>");
        }
    }
